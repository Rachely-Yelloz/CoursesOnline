...
export class CoursesService {
  ...
  getAllCourses(): Observable<Course[]> {
    ...
    return this.http.get<Course[]>(this.apiUrl, this.getHttpOptions()).pipe(
      catchError(error => throwError(() => new Error('Failed to fetch courses')))
    );
  }
  ...
  getCoursesWithTeachers(): Observable<Course[]> {
    return this.getAllCourses().pipe(
      ...
      catchError(error => throwError(() => new Error('Failed to fetch courses with teachers')))
    );
  }

  getCourseDetailsById(id: number): Observable<Course> {
    if (!this.getToken()) return EMPTY;
    return this.http.get<Course>(`${this.apiUrl}/${id}`, this.getHttpOptions()).pipe(
      catchError(error => throwError(() => new Error('Failed to fetch course')))
    );
  }

  getCourseLessonsById(id: number): Observable<Lesson[]> {
    if (!this.getToken()) return EMPTY;
    return this.http.get<Lesson[]>(`${this.apiUrl}/${id}/lessons`, this.getHttpOptions()).pipe(
      catchError(error => throwError(() => new Error('Failed to fetch lessons')))
    );
  }

  addStudentToCourse(courseId: number, userId: number): Observable<any> {
    if (!this.getToken()) return EMPTY;
    return this.http.post(`${this.apiUrl}/${courseId}/enroll`, { userId }, this.getHttpOptions()).pipe(
      catchError(error => throwError(() => new Error('Failed to enroll student')))
    );
  }

  createLesson(courseId: number, title: string, content: string): Observable<{ message: string; lessonId: number }> {
    if (!this.getToken()) return EMPTY;
    return this.http.post<{ message: string; lessonId: number }>(
  ...
  createCourse(title: string, description: string, teacherId: any): Observable<{ message: string; courseId: number }> {
    if (!this.getToken()) return EMPTY;
    return this.http.post<{ message: string; courseId: number }>(
      this.apiUrl,
      { title, description, teacherId },
      this.getHttpOptions()
    );
